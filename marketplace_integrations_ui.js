  ...
  async function renderIntegrations(){
    ...
    try{
      ...
      '<div class="dy-int-grid">'+
        ...
        '<section class="dy-int-card"><div class="dy-int-icon">💳</div><div><h2>Mercado Pago</h2><p>OAuth de vendedores, Checkout Pro y webhook.</p><div class="dy-int-pills">'+pill(d.mercadopago?.oauth,'OAuth')+pill(d.mercadopago?.webhook,'Webhook')+pill(d.mercadopago?.access_token,'Token plataforma')+'</div><small>'+(mpReady?'Infraestructura lista. Falta conectar un vendedor TEST para la prueba punta a punta.':'Configuración incompleta en Render.')+'</small></div></section>'+
