...
(function(){
  ...
  window.enableDatoYaNotifications=async function(){
    ...
    try{
      ...
      if(p==='granted'){toast('🔔 Notificaciones activadas');return true;}
      toast('Las notificaciones del sistema están bloqueadas en el navegador.');
    }catch(e){toast('No se pudo activar las notificaciones.');}
    return false;
  };
  function ensureNotificationButton(){
    ...
    b.id='datoya-notify-btn';
    b.className='btn btn-ghost btn-sm';
    b.textContent='🔔 Activar avisos';
    b.onclick=()=>window.enableDatoYaNotifications();
    area.appendChild(b);
  }
  async function checkDatoYaNotifications(){
    ...
    try{
      ...
      const newest=rows[0];
      if(!lastNotificationId){lastNotificationId=Number(newest.id||0);return;}
      ...
      if(fresh.length){
        ...
        if(notificationSupported()&&Notification.permission==='granted'){
          try{new Notification('DatoYa — Nueva solicitud',{body:solicitud.text,tag:'datoya-'+solicitud.id});}catch(e){}
        }
        if(ME){ME.unread_notifications=(Number(ME.unread_notifications)||0)+fresh.length;renderAuthArea();}
      }
    }catch(e){}
  }
  function startDatoYaNotifications(){
    if(notificationStarted)return;
    notificationStarted=true;
    ensureNotificationButton();
    checkDatoYaNotifications();
    notificationTimer=setInterval(checkDatoYaNotifications,10000);
  }
  const oldRefreshMe=window.refreshMe;
  if(typeof oldRefreshMe==='function'){
    window.refreshMe=async function(){const r=await oldRefreshMe();ensureNotificationButton();startDatoYaNotifications();return r;};
  }
  setTimeout(()=>{ensureNotificationButton();if(ME)startDatoYaNotifications();},1500);
})();
`;
  }
