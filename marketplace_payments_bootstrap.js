...
app.post('/api/orders/:id/mercadopago/checkout',auth,async(req,res)=>{try{
  ...
  const checkout=modeInfo.mode==='test'?mp.sandbox_init_point:mp.init_point;
  if(!mp.id||!checkout)return res.status(502).json({error:modeInfo.mode==='test'?'Mercado Pago no devolvió una URL Sandbox de prueba':'Mercado Pago no devolvió una URL de pago'});
  if(modeInfo.mode==='test'){let __mpCheckoutUrl;try{__mpCheckoutUrl=new URL(String(checkout));}catch(_){return res.status(502).json({error:'Mercado Pago devolvió una URL TEST inválida'});}const __mpHost=String(__mpCheckoutUrl.hostname||'').toLowerCase();const __mpAllowed=__mpCheckoutUrl.protocol==='https:'&&(__mpHost==='sandbox.mercadopago.cl'||__mpHost.endsWith('.sandbox.mercadopago.cl')||__mpHost==='sandbox.mercadopago.com'||__mpHost.endsWith('.sandbox.mercadopago.com'));if(!__mpAllowed)return res.status(502).json({error:'Mercado Pago devolvió una URL que no pertenece al Sandbox y DatoYa la bloqueó'});}
