...
(() => {
  ...
  function button(text, fn, cls='btn btn-outline btn-sm') {
  ...
  function evidenceUrl(storageKey) {
    return '/uploads/job-evidence/' + String(storageKey || '').split('/').filter(Boolean).map(encodeURIComponent).join('/');
  }
  ...
  async function render(force=false) {
    ...
    if (!view) return;
    const existing=document.getElementById('datoya-evidence-panel');
    ...
    try {
      ...
      for (const job of jobs.slice(0,20)) {
        let evidence=[];
        try { evidence=(await api('/jobs/'+job.id+'/evidence')).evidence || []; }
        catch (_) { continue; }

        const wrap=document.createElement('article');
        wrap.style.cssText='border:1px solid #dbe4f0;border-radius:14px;padding:14px;margin:10px 0;background:#fff';
        ...
        for (const stage of stages) {
          ...
          sec.innerHTML='<b>'+stage+'</b>';
          const row=document.createElement('div');
          row.style.cssText='display:flex;gap:8px;overflow:auto;margin-top:6px';
          ...
          for (const e of items) {
            ...
            link.href=evidenceUrl(e.storage_key);
            link.target='_blank';
            link.rel='noopener';
            const img=document.createElement('img');
            img.src=link.href;
            img.alt=stage+' '+(e.original_name||'evidencia');
            img.loading='lazy';
            img.style.cssText='width:96px;height:96px;object-fit:cover;border-radius:10px;border:1px solid #cbd5e1';
            ...
            const b=button('＋ '+stage, async()=>{
              ...
              input.onchange=async()=>{
                ...
                for (const file of Array.from(input.files||[]).slice(0,3)) {
                  ...
                  try {
                    await api('/jobs/'+job.id+'/evidence',{method:'POST',body:{stage,mime_type:file.type,original_name:file.name,data}});
                    uploaded++;
                  } catch(e) { alert(e.message); }
                }
                if (uploaded) await render(true);
              };
              input.click();
            });
      ...
      panel.id='datoya-evidence-panel';
      ...
      // Evidencias es un complemento: si falla, la lista principal de trabajos debe seguir disponible.
    } finally {
      rendering=false;
    }
  }
  ...
  function schedule() {
    clearTimeout(timer);
    timer=setTimeout(()=>render(false),140);
  }

  window.addEventListener('hashchange', schedule);
  new MutationObserver(()=>{
    if(location.hash.startsWith('#/trabajos') && !document.getElementById('datoya-evidence-panel')) schedule();
