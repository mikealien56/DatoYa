...
function translateCommon(sql) {
  ...
  s = s.replace(
    /\bsubstr\s*\(\s*((?:[A-Za-z0-9_]+\.)?(?:created_at|updated_at|expires_at|read_at|started_at|resolved_at|reviewed_at|requested_at|completed_at|verified_at|decided_at|arrival_at|departed_at|ended_at|last_seen_at|location_updated_at|promo_starts_at|promo_ends_at))\s*,/gi,
    'substr(($1)::text,'
  );
  s = s.replace(/\b(created_at|updated_at|expires_at|read_at|started_at|resolved_at|reviewed_at|requested_at|completed_at|verified_at|decided_at|arrival_at|departed_at|ended_at|last_seen_at|location_updated_at|promo_starts_at|promo_ends_at)\s+TEXT\b/gi, '$1 TIMESTAMPTZ');
