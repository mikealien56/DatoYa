...
(() => {
  ...
  function latestText(latest){
    ...
    const coords=Number(latest.lat).toFixed(5)+', '+Number(latest.lng).toFixed(5);
    const time=latest.created_at?new Date(latest.created_at).toLocaleTimeString('es-CL',{hour:'2-digit',minute:'2-digit'}):'';
  ...
  async function render(){
    ...
    try{
      ...
      for(const job of (jobs||[]).slice(0,20)){
        ...
        const s=travel.session; const latest=travel.latest||null; const actions=[];
      ...
      panel.querySelectorAll('[data-gps-update]').forEach(b=>b.onclick=async()=>{try{const p=await locate();await json('/jobs/'+b.dataset.gpsUpdate+'/travel/location',{method:'POST',body:p});alert('Ubicación actualizada.');location.reload();}catch(e){alert(e.message)}});
  ...
  addEventListener('hashchange',schedule); new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true});
  setInterval(()=>{if(location.hash.startsWith('#/trabajos')){rendered=false;render();}},15000);
  schedule();
})();
