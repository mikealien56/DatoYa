...
grep -q "app.post('/api/orders/:id/khipu/checkout',auth,__dyRequireCustomerAccount" marketplace_account_separation_bootstrap.js || fail 'Checkout comprador no exige cuenta cliente'
grep -q 'data-account-type="customer"' marketplace_account_ui.js || fail 'Registro no ofrece cuenta customer'
grep -q 'data-account-type="business"' marketplace_account_ui.js || fail 'Registro no ofrece cuenta business'
grep -q "account_type:type" marketplace_account_ui.js || fail 'Registro no envía el tipo de cuenta elegido'
