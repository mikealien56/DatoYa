...
(()=>{
  ...
  function shell(body){
    view.innerHTML=`<div class="dy-admin-support">
      <div class="row between" style="gap:12px;align-items:center;margin-bottom:14px">
        <div><div class="small muted">Panel Admin</div><h2 style="margin:2px 0">📨 Casos de soporte</h2><p class="small muted" style="margin:0">Busca por número de caso, correo, nombre o asunto.</p></div>
  ...
  function filters(){
    return `<div class="card" style="padding:14px;margin-bottom:14px">
      <div style="display:grid;grid-template-columns:minmax(0,1fr) 180px 190px;gap:10px">
        <input id="dy-support-search" placeholder="Buscar caso, correo, nombre o asunto">
  ...
  function card(c){
    const searchable=h([c.case_ref,c.email,c.name,c.subject,c.category].join(' ').toLowerCase());
    return `<article class="card dy-support-case-card" data-status="${h(c.status)}" data-category="${h(c.category)}" data-search="${searchable}" style="padding:14px">
      ...
      <div style="margin-top:10px"><b>${h(c.subject)}</b><div class="small muted" style="margin-top:2px">${h(c.category)} · ${h(c.name)}</div></div>
      ...
      <div class="row between" style="gap:8px;align-items:center"><span class="small muted">${c.delivery_status==='sent'?'✉️ Correo enviado':c.delivery_status==='failed'?'⚠️ Falló el correo':'⏳ Correo pendiente'}</span><button class="btn btn-primary btn-sm" onclick="dyOpenSupportCase(${Number(c.id)})">Ver caso</button></div>
    </article>`;
  }
  ...
  window.dyOpenSupportCase=async function(id){
    try{
      const r=await api('/admin/support-cases/'+id),c=r.case;
      const replySubject=encodeURIComponent('Re: ['+c.case_ref+'] '+c.subject);
      openModal(`<div class="dy-support-admin-modal">
        ...
        <div class="card" style="padding:12px;margin-top:12px">
          <div class="small"><b>Nombre:</b> ${h(c.name)}</div><div class="small"><b>Correo:</b> ${h(c.email)}</div><div class="small"><b>Categoría:</b> ${h(c.category)}</div><div class="small"><b>Creado:</b> ${date(c.created_at)}</div><div class="small"><b>Entrega email:</b> ${h(c.delivery_status)}</div>
        </div>
        <div class="card" style="padding:12px"><b>${h(c.subject)}</b><p style="white-space:pre-wrap">${h(c.message)}</p></div>
        <div class="field"><label>Estado</label><select id="dy-support-case-status"><option value="new" ${c.status==='new'?'selected':''}>Nuevo</option><option value="in_progress" ${c.status==='in_progress'?'selected':''}>En revisión</option><option value="resolved" ${c.status==='resolved'?'selected':''}>Resuelto</option></select></div>
        <div class="field"><label>Notas internas</label><textarea id="dy-support-case-notes" rows="5" maxlength="4000" placeholder="Notas solo visibles para administración">${h(c.admin_notes||'')}</textarea></div>
        <div class="row wrap" style="margin-top:12px">
          <button class="btn btn-primary" onclick="dySaveSupportCase(${Number(c.id)})">Guardar cambios</button>
          <a class="btn btn-outline" href="mailto:${h(c.email)}?subject=${replySubject}">Responder por correo</a>
        </div>
      </div>`);
