// DatoYa — API de evidencias de trabajos (DEMO)
// Mantiene las evidencias como registros inmutables y separa storage de la lógica de negocio.
const fs = require('fs');
...
function evidenceRows(jobId) {
  return db.prepare(\`SELECT e.id,e.job_id,e.uploader_user_id,e.stage,e.original_name,e.mime_type,e.size_bytes,e.latitude,e.longitude,e.accuracy_m,e.note,e.created_at,u.name AS uploader_name
...
app.post('/api/jobs/:id/evidence', auth, (req,res) => {
  ...
  const key = job.id + '/' + crypto.randomUUID() + '.' + MIME_EXT[mime];
  ...
  const acc = body.accuracy_m == null ? null : Number(body.accuracy_m);
  const id = db.prepare(\`INSERT INTO job_evidence(job_id,uploader_user_id,stage,storage_key,original_name,mime_type,size_bytes,latitude,longitude,accuracy_m,note) VALUES(?,?,?,?,?,?,?,?,?,?,?)\`)
    .run(job.id,req.user.id,stage,key,name,mime,buffer.length,Number.isFinite(lat)?lat:null,Number.isFinite(lng)?lng:null,Number.isFinite(acc)?acc:null,note || null).lastInsertRowid;
  if (typeof addJobEvent === 'function') {
    try { addJobEvent(job.id, 'evidence_uploaded', req.user.id, { evidence_id:id, stage }); } catch (_) {}
  }
...
fs.readFileSync = function(file, options) {
  ...
  if (path.resolve(String(file)) === path.resolve(serverFile) && typeof value === 'string') return injectEvidence(value);
  return value;
};
try {
