...
const indexPath=path.join(pub,'index.html');if(fs.existsSync(indexPath)){let html=fs.readFileSync(indexPath,'utf8');html=html.replace(/<script src="\/legal_final_ui\.js(?:\?v=\d+)?"><\/script>\s*/g,'');html=html.replace('</body>','<script src="/legal_final_ui.js?v=2"></script>\n</body>');fs.writeFileSync(indexPath,html);}
