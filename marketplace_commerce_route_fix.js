...
if(fs.existsSync(file)){
  ...
  if(start>=0&&end>start){
    const route=[
      ...
      "    db.prepare('INSERT INTO commerce_orders(reference,user_id,business_id,status,fulfillment_method,customer_name,customer_phone,delivery_address,notes,subtotal,delivery_fee,delivery_distance_km,total,payment_method,payment_status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(ref,req.user.id,b.id,'new',method,customerName,customerPhone,address,notes,subtotal,deliveryFee,deliveryDistanceKm,total,'arrange','pending',now,now);",
