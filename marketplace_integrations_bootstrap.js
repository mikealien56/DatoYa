...
if(source.includes(orderNeedle)&&!source.includes("Nuevo pedido en DatoYa")){
  const orderReplacement=orderNeedle+"\n  const __ownerEmail=(db.prepare('SELECT email FROM users WHERE id=?').get(b.owner_user_id)||{}).email||null;\n  if(__ownerEmail)Promise.resolve(__dyCommerceEmail(__ownerEmail,'Nuevo pedido en DatoYa','Recibiste un nuevo pedido','<p>Pedido <b>'+ref+'</b> por <b>'+fmtCLP(total)+'</b>.</p><p>Revísalo y actualiza su estado desde tu panel.</p>','Ver pedido','/#/mi-negocio-pedidos/'+b.id)).catch(()=>{});\n  if(req.user.email)Promise.resolve(__dyCommerceEmail(req.user.email,'Pedido recibido por DatoYa','Tu pedido fue enviado','<p>Enviamos el pedido <b>'+ref+'</b> a <b>'+b.name+'</b> por <b>'+fmtCLP(total)+'</b>.</p>','Ver mi pedido','/#/pedidos/'+orderId)).catch(()=>{});";
  source=source.replace(orderNeedle,orderReplacement);
}

const statusNeedle="const __orderStateLabel={confirmed:'fue confirmado',preparing:'está en preparación',ready:o.fulfillment_method==='delivery'?'está listo para despacho':'está listo para retirar',completed:'fue completado',cancelled:'fue cancelado'}[next]||('cambió a '+next);notify(o.user_id,'pedido','Tu pedido '+o.reference+' '+__orderStateLabel+'.','#/pedidos/'+o.id);res.json({ok:true});";
if(source.includes(statusNeedle)&&!source.includes("Actualización de tu pedido DatoYa")){
  const statusReplacement="const __orderStateLabel={confirmed:'fue confirmado',preparing:'está en preparación',ready:o.fulfillment_method==='delivery'?'está listo para despacho':'está listo para retirar',completed:'fue completado',cancelled:'fue cancelado'}[next]||('cambió a '+next);notify(o.user_id,'pedido','Tu pedido '+o.reference+' '+__orderStateLabel+'.','#/pedidos/'+o.id);const __customer=(db.prepare('SELECT email,name FROM users WHERE id=?').get(o.user_id)||{});if(__customer.email)Promise.resolve(__dyCommerceEmail(__customer.email,'Actualización de tu pedido DatoYa','Tu pedido cambió de estado','<p>Tu pedido <b>'+o.reference+'</b> '+__orderStateLabel+'.</p>','Ver pedido','/#/pedidos/'+o.id)).catch(()=>{});res.json({ok:true});";
