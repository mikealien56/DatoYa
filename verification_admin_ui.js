...
(function(){
  ...
  const adminMenu=`<div class="tabs admin-tabs" style="display:flex;flex-wrap:wrap;overflow:visible;gap:6px"><button onclick="location.hash='#/admin'">📊 Resumen</button><button onclick="location.hash='#/admin/usuarios'">👥 Usuarios</button><button onclick="location.hash='#/admin/trabajadores'">🔧 Profesionales</button><button onclick="location.hash='#/admin/categorias'">🧰 Categorías</button><button onclick="location.hash='#/admin/trabajos'">🧾 Trabajos</button><button onclick="location.hash='#/admin/verificaciones'">🪪 Verificaciones</button><button onclick="location.hash='#/admin/reclamos'">⚑ Reclamos</button><button onclick="location.hash='#/admin/disputas'">⚖️ Disputas</button><button onclick="location.hash='#/admin/auditoria'">🕘 Auditoría</button><button onclick="location.hash='#/admin/ganancias'">💰 Ganancias</button><button onclick="location.hash='#/admin/retiros'">💸 Retiros</button><button onclick="location.hash='#/admin/banco'">🏦 Banco</button><button onclick="location.hash='#/admin/mensajes'">💬 Mensajes</button><button onclick="location.hash='#/admin/suscripciones'">⭐ Suscripciones</button><button onclick="location.hash='#/admin/configuracion'">⚙️ Configuración</button></div>`;
  function statusClass(s){return s==='pendiente'?'st-DISPUTA':s==='aprobada'?'st-FINALIZADO':'st-CANCELADO'}
  ...
  function renderVerificationDetail(c){
    ...
    const docs=`<div class="card"><h3>📄 Antecedentes registrados</h3><div class="small"><b>Tipo:</b> ${esc(v.document_type||v.type||'No indicado')}</div><div class="small" style="margin-top:6px"><b>Referencia:</b><br>${refs}</div>${v.needs_documents?'<div class="lock-note" style="margin-top:10px">⏳ Esperando antecedentes adicionales del profesional.</div>':''}<div class="lock-note" style="margin-top:10px">Los archivos de identidad reales deben almacenarse en un repositorio privado antes de producción. Esta vista no expone documentos públicamente.</div></div>`;
    const actions=v.status==='pendiente'?`<div class="card"><h3>📋 Acción administrativa</h3><div class="row wrap"><button class="btn btn-primary btn-sm" onclick="requestVerificationDocs(${v.id})">📎 Solicitar más antecedentes</button><button class="btn btn-green btn-sm" onclick="reviewVerification(${v.id},'aprobar')">✓ Aprobar</button><button class="btn btn-danger btn-sm" onclick="reviewVerification(${v.id},'rechazar')">Rechazar</button></div></div>`:'';
    return `<div class="card" style="margin-top:12px"><div class="row between"><div><h3 style="margin-bottom:3px">🪪 ${esc(v.name)}</h3><div class="small muted">${esc(v.oficio||'Profesional')} · ${esc(v.email||'')}</div></div><span class="status-tag ${statusClass(v.status)}">${v.needs_documents?'pendiente de antecedentes':esc(v.status||'pendiente')}</span></div><div class="small" style="margin-top:10px"><b>Ticket:</b> ${esc(v.ticket||('VER-'+v.id))}</div>${docs}${actions}<div class="card"><h3>🕘 Historial de revisión</h3>${historyHtml(c.history)}</div></div>`;
  }
  ...
  routes.admin=async function(tab='dashboard'){
    ...
    try{
      ...
      const html=requests.map(v=>`<div class="card"><div class="row between"><div><b>🪪 ${esc(v.name)}</b><div class="small muted">${esc(v.oficio||'Profesional')} · ${esc(v.email||'')}</div></div><span class="status-tag ${statusClass(v.status)}">${esc(v.status||'pendiente')}</span></div><div class="small" style="margin-top:8px"><b>Ticket:</b> ${esc(v.ticket||('VER-'+v.id))} · <b>Documento:</b> ${esc(v.document_type||v.type||'No indicado')}</div><div class="small muted">${fmtHora(v.created_at)}</div><div id="verification-detail-${v.id}"></div><div class="row" style="margin-top:10px"><button class="btn btn-primary btn-sm" onclick="openAdminVerification(${v.id})">📁 Revisar expediente</button>${v.status==='pendiente'?`<button class="btn btn-outline btn-sm" onclick="requestVerificationDocs(${v.id})">📎 Pedir antecedentes</button>`:''}</div></div>`).join('')||'<div class="empty"><b>✓</b>No hay solicitudes de verificación.</div>';
      view.innerHTML=`<h2 class="section-title" style="margin-top:0">🛡️ Verificaciones profesionales</h2>${adminMenu}${html}`;
