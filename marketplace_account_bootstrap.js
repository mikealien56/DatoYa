// DatoYa — cuenta única + dominio comercial mínimo para negocios/emprendimientos.
// Se ejecuta antes de territory_start para que las rutas queden disponibles al iniciar server.js.
const fs=require('fs');
...
const marketCategories=[
  ['comida','Restaurantes','🍽️',10],
  ['comida-rapida','Comida rápida','🍔',20],
  ['cafeterias','Cafeterías','☕',30],
  ['panaderia','Panaderías','🥐',40],
  ['pastelerias','Pastelerías','🎂',50],
  ['tiendas','Tiendas','🛍️',60],
  ['almacenes','Almacenes','🏪',70],
  ['minimarkets','Minimarkets','🧺',80],
  ['farmacia','Farmacias','✚',90],
  ['ferreterias','Ferreterías','🔩',100],
  ['mascotas','Mascotas','🐾',110],
  ['belleza','Belleza','✂️',120],
  ['ropa','Ropa','👕',130],
  ['regalos','Regalos','🎁',140],
  ['librerias','Librerías','📚',150],
  ['tecnologia','Tecnología','💻',160],
  ['hogar','Hogar','🏠',170],
  ['artesania','Artesanía','🧶',180],
  ['otros','Otros','➕',999]
];
...
// Las cuentas nuevas son cuentas DatoYa únicas. No se crean nuevos perfiles de trabajador.
source=source.replace("const finalRole = role === 'trabajador' ? 'trabajador' : 'cliente';","const finalRole = 'cliente';");

if(!source.includes('DATOYA MARKETPLACE ACCOUNT V1')){
  const injection=`
// ============ DATOYA MARKETPLACE ACCOUNT V1 ============
function __marketSlug(value){
  return String(value||'negocio').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,60)||'negocio';
}
...
app.get('/api/market/categories',(req,res)=>{
  res.json({categories:db.prepare('SELECT id,slug,name,icon,sort_order FROM market_categories WHERE active=1 ORDER BY sort_order,name').all()});
});
...
app.post('/api/businesses',auth,(req,res)=>{
  ...
  const validCats=db.prepare('SELECT id FROM market_categories WHERE active=1 AND id IN ('+categoryIds.map(()=>'?').join(',')+')').all(...categoryIds).map(x=>Number(x.id));
  if(validCats.length!==categoryIds.length) return res.status(400).json({error:'Una de las categorías no es válida'});
  const dup=db.prepare('SELECT id FROM businesses WHERE owner_user_id=? AND lower(name)=lower(?) LIMIT 1').get(req.user.id,name);
  if(dup) return res.status(409).json({error:'Ya tienes un negocio con ese nombre'});
  ...
  const id=tx();
  res.json({ok:true,business:__marketBusinessRow(db.prepare('SELECT b.*,c.name AS comuna,r.name AS region FROM businesses b LEFT JOIN comunas c ON c.id=b.comuna_id LEFT JOIN regions r ON r.id=c.region_id WHERE b.id=?').get(id))});
});
app.put('/api/account/profile',auth,(req,res)=>{
  const name=String(req.body?.name||'').trim();
  const phone=String(req.body?.phone||'').trim();
  const comunaId=Number(req.body?.comuna_id||0);
...
// ============ FIN DATOYA MARKETPLACE ACCOUNT V1 ============
`;
  source=source.replace('// ============ CATÁLOGOS ============',injection+'\n// ============ CATÁLOGOS ============');
}
fs.writeFileSync(serverPath,source);
console.log('[DatoYa] Cuenta única y dominio de negocios preparados.');
