    ...
    function render(){
      ...
      view.innerHTML=`<div class="dy-business-wizard"><a href="#/perfil" class="dy-wizard-back">← Mi cuenta</a><div class="dy-wizard-head"><span>REGISTRAR NEGOCIO</span><h1>${step===1?'¿Qué tipo de negocio tienes?':step===2?'Cuéntanos sobre tu negocio':step===3?'¿Dónde está ubicado?':step===4?'Horarios y atención':'Revisa antes de enviar'}</h1><div class="dy-progress"><i style="width:${pct}%"></i></div><small>Paso ${step} de 5</small></div><div class="dy-wizard-card">${step===1?`
        ...
        <section class="dy-hours-main-card dy-signup-hours">
          <div class="dy-hours-toolbar"><div><b>Horario semanal</b><small>Activa los días que atiendes. Los demás quedarán como Cerrado.</small></div><div><button type="button" class="btn btn-outline btn-sm" id="dy-signup-weekdays">Lun–Vie 09:00–18:00</button><button type="button" class="btn btn-outline btn-sm" id="dy-signup-copy-weekdays">Copiar lunes a Lun–Vie</button><button type="button" class="btn btn-outline btn-sm" id="dy-signup-close-weekend">Cerrar fin de semana</button></div></div>
          <div class="dy-hours-days">${hourDays.map(([key,label])=>{const v=(draft.hours_schedule?.[key]||[])[0],on=!!v;return `<article class="dy-hours-day-card ${on?'open-day':'closed-day'}" data-signup-hour-day="${key}"><div class="dy-hours-day-head"><div><b>${label}</b><small class="dy-hours-day-state">${on?'Abierto':'Cerrado'}</small></div><label class="dy-switch"><input type="checkbox" name="${key}_enabled" ${on?'checked':''}><span></span></label></div><div class="dy-hours-times ${on?'':'disabled'}"><label><span>Abre</span><input type="time" name="${key}_open" value="${h(v?.open||'09:00')}" ${on?'':'disabled'}></label><span class="dy-hours-arrow">→</span><label><span>Cierra</span><input type="time" name="${key}_close" value="${h(v?.close||'18:00')}" ${on?'':'disabled'}></label></div></article>`;}).join('')}</div>
      ...
      function syncSignupHourDay(key){
        ...
        const open=document.querySelector('[name="'+key+'_open"]'),close=document.querySelector('[name="'+key+'_close"]');if(open)open.disabled=!on;if(close)close.disabled=!on;
      }
      hourDays.forEach(([key])=>document.querySelector('[name="'+key+'_enabled"]')?.addEventListener('change',()=>syncSignupHourDay(key)));
      document.getElementById('dy-signup-weekdays')?.addEventListener('click',()=>{for(const [key] of hourDays){const enabled=document.querySelector('[name="'+key+'_enabled"]'),open=document.querySelector('[name="'+key+'_open"]'),close=document.querySelector('[name="'+key+'_close"]');if(!enabled)continue;const weekday=['mon','tue','wed','thu','fri'].includes(key);enabled.checked=weekday;if(open)open.value='09:00';if(close)close.value='18:00';syncSignupHourDay(key);}});
