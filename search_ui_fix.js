...
(() => {
  ...
  routes.buscar=async function(){
    ...
    const [catData,comunaData]=await Promise.all([
      api('/categories'),
      api('/comunas')
    ]);
      ...
      <div class="card" style="margin-top:12px">
        <form id="datoya-search-form">
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px">
            ...
            <div class="field"><label>Comuna</label><select name="comuna"><option value="">Todas</option>${comunas.map(c=>`<option value="${c.id}" ${String(q.get('comuna')||'')===String(c.id)?'selected':''}>${escSearch(c.name)}${c.region?' — '+escSearch(c.region):''}</option>`).join('')}</select></div>
          ...
          <div class="row wrap" style="margin-top:8px"><button class="btn btn-primary" type="submit">Aplicar filtros</button><a class="btn btn-ghost" href="#/buscar">Limpiar</a></div>
        </form>
      </div>
    ...
    document.getElementById('datoya-search-form')?.addEventListener('submit',e=>{
      ...
      for(const key of ['q','cat','comuna','status','rating','max_price']){
