...
const amounts=[[85000,8500],[150000,15000],[65000,6500],[210000,21000],[95000,9500]];for(const [gross,comm] of amounts){const exists=db.prepare('SELECT id FROM payments WHERE amount=? AND commission=? AND provider=\'DEMO\'').get(gross,comm);if(!exists){db.prepare('INSERT INTO payments(job_id,amount,commission,worker_amount,method,provider,status) VALUES(?,?,?,?,?,?,?)').run(job,gross,comm,gross-comm,'transfer','DEMO','demo_completado')}}

// Retiros, banco, PRO, verificación y denuncias para poblar el panel.
for(const p of db.prepare('SELECT id FROM worker_profiles ORDER BY id LIMIT 3').all()){
  const exists=db.prepare("SELECT id FROM payout_requests WHERE worker_id=? AND amount=45000 AND status='pagado'").get(p.id);
  if(!exists)db.prepare('INSERT INTO payout_requests(worker_id,amount,status) VALUES(?,?,?)').run(p.id,45000,'pagado');
}
...
const sub=db.prepare("SELECT id FROM subscriptions WHERE worker_id=? AND status='activa'").get(wp.id);if(!sub)db.prepare("INSERT INTO subscriptions(worker_id,plan,status,amount) VALUES(?,?,?,?)").run(wp.id,'MENSUAL','activa',9990);
