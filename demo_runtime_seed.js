...
function demoUser(email,name,role,comunaName='Santiago',regionName='Metropolitana de Santiago',roman='RM'){
 ...
 return db.prepare('INSERT INTO users(email,password_hash,name,role,comuna_id,is_demo) VALUES(?,?,?,?,?,1)').run(email,hashPassword('demo1234'),name,role,comuna(comunaName,regionName,roman)).lastInsertRowid;
}
...
function payment(amount){
 const commission=Math.round(amount*0.10),exists=db.prepare('SELECT id FROM payments WHERE job_id=?').get(jobId);
 if(!exists){db.prepare('INSERT INTO payments(job_id,amount,commission,worker_amount,method,provider,status) VALUES(?,?,?,?,?,?,?)').run(jobId,amount,commission,amount-commission,'tarjeta','DEMO','demo_completado');if(!db.prepare('SELECT id FROM commissions WHERE job_id=?').get(jobId))db.prepare('INSERT INTO commissions(job_id,pct,amount) VALUES(?,?,?)').run(jobId,10,commission);}
}
payment(120000);
for(const row of db.prepare('SELECT id FROM worker_profiles ORDER BY id LIMIT 3').all()){if(!db.prepare("SELECT id FROM payout_requests WHERE worker_id=? AND amount=45000 AND status='pagado'").get(row.id))db.prepare("INSERT INTO payout_requests(worker_id,amount,status) VALUES(?,?,?)").run(row.id,45000,'pagado');}
...
if(db.prepare('SELECT COUNT(*) c FROM reports').get().c===0){db.prepare('INSERT INTO reports(reporter_id,target_type,target_id,reason,details) VALUES(?,?,?,?,?)').run(don,'usuario',worker,'Problema con profesional DEMO','Expediente ficticio: Cliente → Profesional.');db.prepare('INSERT INTO reports(reporter_id,target_type,target_id,reason,details) VALUES(?,?,?,?,?)').run(worker,'usuario',don,'Conducta del cliente DEMO','Expediente ficticio: Profesional → Cliente.');}
