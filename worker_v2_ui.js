...
(function(){
  ...
  routes.perfil=async function(){
    ...
    const gps=document.createElement('div'); gps.className='card';
    gps.innerHTML='<h3>📍 Aparecer cerca de clientes</h3><p class="small muted">Si activa su ubicación, DatoYa podrá mostrarle como profesional cercano a clientes que buscan su especialidad. Solo mostramos una distancia aproximada; nunca su coordenada exacta.</p><p class="small">'+(location.location?'Última ubicación actualizada: '+fmtHora(location.location.updated_at)+'.':'Todavía no tiene una ubicación GPS registrada.')+'</p><button id="worker-gps-update" class="btn btn-primary btn-block">📍 '+(location.location?'Actualizar mi ubicación':'Usar mi ubicación')+'</button>'+(location.location?'<button id="worker-gps-delete" class="btn btn-ghost btn-block">Dejar de usar GPS</button>':'');
