...
(() => {
  ...
  routes.negocio=async function(identifier){
    ...
    try{
      ...
      const items=b.products||[],wa=String(b.whatsapp||b.phone||'').replace(/\D/g,'');
      const publicPlace=b.address&&b.public_address_mode==='exact'?b.address:[b.sector,b.comuna].filter(Boolean).join(', ');
      ...
      view.innerHTML=`<div class="dy-public-page dy-store-page">
        ...
        <div class="dy-growth-actions">${wa?`<a class="btn btn-primary" id="dy-growth-wa" href="https://wa.me/${h(wa)}?text=${encodeURIComponent('Hola, vi tu negocio en DatoYa y quería consultar por…')}" target="_blank" rel="noopener">💬 Consultar por WhatsApp</a>`:''}<button class="btn btn-outline" id="dy-growth-share">↗ Compartir negocio</button></div>
        ...
        <section><div class="dy-section-head"><div><h2>Productos (${items.length})</h2><p>Catálogo publicado por el negocio.</p></div></div><div class="dy-store-products">${items.length?items.map(p=>`<article class="dy-store-product dy-growth-product" data-growth-product="${Number(p.id)}"><div>${p.image_data?`<img src="${p.image_data}" alt="${h(p.name)}" loading="lazy">`:'<div class="dy-real-placeholder">📦</div>'}</div><div><span>${h(p.category_icon||'')} ${h(p.category_name||'')}</span><h3>${h(p.name)}</h3><p>${h(p.description||'')}</p><div class="dy-product-public-price">${p.promo_price?`<s>${money(p.price)}</s>`:''}<strong>${money(p.promo_price||p.price)}</strong>${p.stock_tracking?`<em>${Number(p.stock||0)>0?`${Number(p.stock)} disponibles`:'Sin stock'}</em>`:''}</div><button class="btn btn-outline btn-sm dy-share-product" data-product-id="${Number(p.id)}" data-product-name="${h(p.name)}" data-product-price="${money(p.promo_price||p.price)}">↗ Compartir</button></div></article>`).join(''):'<div class="dy-real-empty"><span>📦</span><b>Sin productos visibles</b></div>'}</div></section>
      </div>`;
  ...
  if(previousMine)routes['mi-negocio']=async function(id){
    ...
    try{
      ...
      if(firstCard&&!document.getElementById('dy-growth-share-card')){
        ...
        firstCard.insertAdjacentHTML('beforebegin',`<section class="dy-business-card dy-growth-share-card" id="dy-growth-share-card"><div><span>COMPARTE TU NEGOCIO</span><h2>🔗 Tu vitrina pública DatoYa</h2><p>Comparte este enlace o muestra el QR en tu local, redes o WhatsApp.</p><code>${h(url)}</code><div class="dy-growth-actions"><button class="btn btn-primary" id="dy-owner-share">Compartir</button><button class="btn btn-outline" id="dy-owner-copy">Copiar enlace</button></div></div><div class="dy-growth-qr"><img src="/api/businesses/${business.id}/qr.svg" alt="QR de ${h(business.name)}"><small>Escanea para abrir el negocio</small></div></section>`);
