...
app.post('/api/requests',auth,requireRole('cliente'),(req,res,next)=>{
 const b=req.body||{},title=String(b.title||'').trim(),description=String(b.description||'').trim(),address=String(b.address_detail||'').trim(),budget=b.budget==null||b.budget===''?null:Number(b.budget);
